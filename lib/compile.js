...
var translate = (function () {
  ...
  function set(node, options, resume, params) {
    visit(node.elts[0], options, function (err, val) {
      if ((typeof val === "undefined" ? "undefined" : _typeof(val)) !== "object" || !val || !val.scale) {
  ...
  function features(node, options, resume) {
    //0 = map, 1 = array
    visit(node.elts[0], options, function (err, val) {
      if ((typeof val === "undefined" ? "undefined" : _typeof(val)) !== "object" || !val || !val.scale) {
        err = err.concat(error("Argument Map invalid.", node.elts[0]));
  ...
  function rotate(node, options, resume) {
    ...
    visit(node.elts[1], options, function (err1, val1) {
      if (val1 instanceof Array && val1.length > 1) {
        ...
        if (val1[2]) {
          //if it's zero as opposed to null we still aren't doing any harm by skipping it.
          if (!isNaN(val1[2])) {
            // if there IS something given for roll it better be a number.
  ...
  function parallels(node, options, resume) {
  ...
  function limit(node, options, resume) {
    //[minlatitude, maxlatitude], [minlongitude, maxlongitude], map
    visit(node.elts[1], options, function (err1, val1) {
      //[minlongitude, maxlongitude]
      if (!(val1 instanceof Array) || +val1[0] < -180 || +val1[0] > +val1[1] || +val1[1] > 180 || +val1[1] < +val1[0]) {
      ...
      visit(node.elts[2], options, function (err2, val2) {
        ...
        if (!(val2 instanceof Array) || +val2[0] < -90 || +val2[0] > +val2[1] || +val2[1] > 90 || +val2[1] < +val2[0]) {
  ...
  function point(node, options, resume) {
    //latitude, longitude
    var ret1 = null;
    visit(node.elts[0], options, function (err1, val1) {
      //longitude
  ...
  function color(node, options, resume) {
    visit(node.elts[0], options, function (err, val) {
      var ret = null;
      if ((typeof val === "undefined" ? "undefined" : _typeof(val)) === "object" && val && (val.key === "point" || val.type === "LineString")) {
        visit(node.elts[1], options, function (err1, val1) {
          ret = colorcheck(val1);
  ...
  function label(node, options, resume) {
    visit(node.elts[0], options, function (err, val) {
      var ret = null;
      if ((typeof val === "undefined" ? "undefined" : _typeof(val)) === "object" && val && val.key === "point") {
        visit(node.elts[1], options, function (err1, val1) {
  ...
  function title(node, options, resume) {
    ...
    visit(node.elts[1], options, function (err1, val1) {
      //pos
      if (typeof val1 === 'string' && labeloptions[val1]) {
        //first word is top, bottom, etc, second is right/left
  ...
  function position(node, options, resume) {
    //position lat long map
    var ret = [];
    visit(node.elts[1], options, function (err1, val1) {
      //longitude
      if (!isNaN(val1)) {
  ...
  function get(node, options, resume) {
    //takes in URL for map
    visit(node.elts[0], options, function (err, val) {
      if (typeof val !== "string" || val.indexOf("http") < 0) {
        //if there's no http or https we can't check protocol, right?
  ...
  function csv(node, options, resume) {
    //takes in 'get {csv here} object' basically
    //0 = map, 1 = csv
  ...
  function info(node, options, resume) {
    //0 = map (with csv), 1 = position
    var opt = {
      "tooltip": 0,
      "top": 1,
      "bottom": 2,
  ...
  function order(node, options, resume) {
    //0 = map (with csv), 1 = descriptor, 2 = order
    visit(node.elts[1], options, function (err1, val1) {
  ...
  function zoom(node, options, resume) {
    //0 = map, 1 = max, 2 = min --- zoom min max map
    visit(node.elts[1], options, function (err1, val1) {
      //max
  ...
  function highlight(node, options, resume) {
    //0 = map, 1 = pairs
    var ret = [];
    visit(node.elts[1], options, function (err1, val1) {
      //[[id, color], [id, color]]
  ...
  function chighlight(node, options, resume) {
    //0 = map, 1 = pairs
    var ret = [];
    visit(node.elts[1], options, function (err1, val1) {
      //[[id, color], [id, color]]
  ...
  function dhighlight(node, options, resume) {
    var ret = {};
    visit(node.elts[1], options, function (err1, val1) {
      //0 = map, 1 = colors, 2 = names
      if (!(val1 instanceof Array)) {
        //[red, blue, someothercolor]
        err1 = err1.concat(error("Argument must be an array.", node.elts[1]));
      }
      visit(node.elts[2], options, function (err2, val2) {
        //[name, name, name]
  ...
  function style(node, options, resume) {
    visit(node.elts[1], options, function (err2, val2) {
      if (val2 instanceof Array && val2.length) {
        visit(node.elts[0], options, function (err1, val1) {
          if ((typeof val1 === "undefined" ? "undefined" : _typeof(val1)) !== "object" || !val1 || !val1.scale) {
            err1 = err1.concat(error("Argument Map invalid.", node.elts[0]));
          } else {
            val2.forEach(function (element, index) {
              if (element.key && element.val) {
                //first tier: title, map, borders, features, info
                unwrap(element, val1);
