...
var translate = (function () {
    		...
    		if(val1 instanceof Array && val2 instanceof Array && val1.length == val2.length){
    			val1.forEach(function (element, index){
    				if(!isNaN(element) && Math.abs(+element) <= 180){
    					if(!isNaN(val2[index]) && Math.abs(+val2[index]) <= 90){
    						coordinates[index] = [+val1[index], +val2[index]];
    					} else {
    			...
    			err2 = err2.concat(error("Argument arrays invalid or mismatched in length.", node.elts[1]));
    		}
    		resume([].concat(err1).concat(err2), {//lines is an array of linestrings.
    			type: "LineString",
  ...
  function map(node, options, resume) {
    //takes in projection, latitude and longitude
    var ret = {
      width: 960,
      height: 550,
      scale: 153,
